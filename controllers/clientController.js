...
exports.getClientById = async (req, res) => {
  ...
  try {
    ...
    if (!client)
      return res.status(400).json({ error: "No se pudo encontrar al cliente" });
...
exports.newClient = async (req, res) => {
  ...
  try {
    const alreadyExist = await Cliente.findOne({
...
exports.updateClientById = async (req, res) => {
  ...
  const { id } = req.params;
  try {
    const { nombre, apellidos, num_telefonico, email, dni, ruc, direccion } =
      req.body;
    const [number] = await Cliente.update(
      { nombre, apellidos, num_telefonico, email, dni, ruc, direccion },
      {
        where: { id },
      }
    );
    ...
    res.status(200).json({ msg: "Cambios guardados correctamente" });
  } catch (error) {
    console.log(error);
    res.status(500).json({ error: "Ocurrió un error" });
  }
