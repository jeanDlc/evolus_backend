...
router.get("/", auth, verifyAuthUser, projectController.getProjects);

router.get("/:id", auth, verifyAuthUser, projectController.getProjectById);
...
router.post(
  ...
  body("fecha_fin")
    .isDate()
    .withMessage("Fecha final no válida")
    .custom((value, { req }) => {
  ...
  body("num_matricula")
    ...
    .escape()
    .isLength({ max: 7 })
    .withMessage("Matrícula no válida"),
  body("monto").isNumeric().withMessage("Monto no válido"),
  body("empleados").isArray({ min: 0 }).withMessage("Empleados no válidos"),
  handlerValidationErrors,
...
router.post(
  ...
  JefeTallerPermission,
  validatePermissions,
  projectController.handleProjectEmpleados
);
router.put(
  "/:id",
  auth,
